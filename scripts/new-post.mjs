...
function parseArgs(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (a === "--category") result.category = args[++i];
...
async function main() {
  ...
  if (!title) {
    console.error(
      '用法: npm run new:post "文章标题" [--slug xxx] [--desc "简介"] [--hero "/uploads/xx.jpg"] [--category "分类"]',
    );
    process.exit(1);
  }
        ...
        if (process.stdin.isTTY) {
          ...
          try {
            const input = await ask(
              rl,
              `无法自动转拼音，请输入 slug（回车使用 ${fallback}）: `,
              fallback,
            );
            fallback = input;
          } finally {
  ...
  let desc = descArg;
  let hero = heroArg;
  const category = categoryArg || "未分类";
